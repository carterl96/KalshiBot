...
def test_hard_price_stop_fires():
    # Entry 0.57, sell-price drops 0.18 below entry -> stop fires.
    m = make_mgr(stop_loss_drop=0.18)
    t, s = _open(m)
    assert m.exit_signal(t, s, 0.40, 0.57, 0.5, 600) is None   # 0.17 drop, no stop
    assert m.exit_signal(t, s, 0.38, 0.57, 0.5, 600) == "stop_loss"  # 0.19 drop -> stop


def test_hard_price_stop_not_triggered_on_small_dip():
    m = make_mgr(stop_loss_drop=0.18)
    t, s = _open(m)
    for _ in range(10):
        # A 0.10 dip — well below stop_loss_drop, should never trigger.
        assert m.exit_signal(t, s, 0.47, 0.57, 0.5, 600) is None
